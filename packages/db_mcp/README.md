...
- `roles.py` — канонический вокабуляр ролей: `BusinessRole`
  (applicant/student/teacher/admin) и `DbPool` (ro/admin/service).
- `access.py` — пулы соединений asyncpg по ролям PostgreSQL и установка
  ...
  `users.id`) резолвится в доменный `internal_id` через роль `app_service`
...
- `manage_user(action, ...)` — чтение/запись учётных записей (`users`) для auth:
  `get`/`list`/`create`/`update`/`deactivate`. Жёстко параметризованные запросы
  через служебную роль `app_service` (не произвольный SQL, не проходит через
  read-only валидатор). Используется приложением для хранения учёток в БД.
