...
Эти модели описывают схему БД и используются для генерации сида. PII-поля
студентов помечены комментарием `# sensitive`: они доступны только роли
администрации (app_admin), для остальных ролей физически скрыты колоночными
грантами и не попадают в описание схемы для LLM. Список PII-колонок для
маскирования описания схемы — в db_mcp/schema.py (SENSITIVE_COLUMNS).
"""
