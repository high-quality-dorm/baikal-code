// Локальный mock-ассистент: фолбэк, когда бэкенд-эндпоинт /api/v1/ask
// недоступен (бэкенд не запущен) и для гостевой сессии (у гостя нет
// Bearer-токена, а /ask требует авторизацию). Фронтенд отвечает
// детерминированно на основе фактов засеянной базы (см. docs/seed.md).
// Интерфейс возврата совпадает с Answer/QueryMeta из
