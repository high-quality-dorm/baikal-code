...
Помимо этого, **публичные агрегатные вью** (`db/04_views.sql`, `v_*`) доступны
всем ролям и гостю: `v_students_total = 500`, суммы по факультетам/статусам
сходятся (например, Обучается 241, Выпускник 231, Отчислен 22, Академический
отпуск 6). Напрямую `students`/`marks` гостю по-прежнему не видны (0 строк).

## Замечания по реализации

- Вставка идёт пачками (`_insert_rows`/`_exec_many`, multi-row `VALUES` и
  `executemany`, пачка ~500 строк) внутри одной транзакции. Это убирает сетевой
  round-trip на каждую строку: на удалённой БД с задержкой ~75 мс/statement
  сид занимает ~10 с вместо десятков минут. Данные не меняются, объёмы те же.
