...
безопасности, живёт здесь. Роль строкой не хранится и **не участвует в доступе**
— скоуп выводится из `student_id`/`staff_id` пользователя (см. «Модель доступа»);
в шлюз роль передаётся только для журнала аудита (`execute_query(..., role)`,
см. ADR 16).
...
- `POST /api/v1/ask` — тул-агент; ответ — **NDJSON-поток**; **гость разрешён**
  (без токена → `user_id=None`). Текст вопроса может нести контекст беседы:
  фронтенд вклеивает предыдущие реплики перед новым вопросом (лимит текста —
  100 000 символов). Запрос ограничен rate limiting (см. ADR 37):
