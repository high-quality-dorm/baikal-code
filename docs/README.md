...
Запущен на https://swe.vipono.online
...
- **Прозрачность ответа:** у каждого ответа раскрывается SQL и метаданные
  (число строк, время, признак обрезки), а агент добавляет блок «Объяснение
  запроса» — какие таблицы, JOIN, фильтры и агрегаты использованы.
- **Веб-интерфейс:** лендинг, чат, вход; гостевой доступ без регистрации;
  встраиваемый чат-виджет для внешних сайтов университета.
