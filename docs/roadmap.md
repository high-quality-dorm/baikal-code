...
| 16 | Frontend | React SPA (Vite) в `frontend/`: лендинг, чат, вход; токены/темы; гостевой доступ; вход через `/auth/login`; `/ask` подключён с Bearer-токеном; mock — фолбэк для гостя и при недоступном бэкенде | (этап работы) |
