...
| 23 | Rate limiting для /ask | `SlidingWindowLimiter` (скользящее окно, in-process) в `core/ratelimit.py`; раздельные лимиты: авторизованный по `user_id`, гость по IP; параметры в `.env` (`RATE_LIMIT_*`); 429 при превышении; лимитер — одиночка в `AppContext`; ADR 37; тесты (188) | (этап 23, текущий) |
...
- **Устранение замечаний бэкенда** (поэтапный план — [fixes.md](fixes.md)):
  rate limit для `/ask`, разрешение PII по RLS-скоупу, приоритет должности в
  `resolve_role`, реальная роль в аудите.
