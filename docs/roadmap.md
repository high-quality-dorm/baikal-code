...
| 27 | Публичные агрегаты по студентам | Вью `v_*` (`db/04_views.sql`): численность студентов (всего, по факультетам/направлениям/группам/статусам/годам, отчисления) без PII — доступна всем, включая гостя; FORCE RLS снят с `students` (владелец считает агрегаты; для `app_ro` RLS не меняется), `marks` — с FORCE; поимённые гранты `v_*` для `app_ro`; `TABLE_META` для LLM; тесты-инварианты (216) | `5e7f29b` |
| 28 | Встраиваемый виджет | Страница `/widget` — плавающий launcher-чат для `<iframe>` на внешних сайтах; гость + кнопка «Войти» (вход в новой вкладке, сессия синхронизируется через `storage`-событие); API-базис из `window.location.origin`; nginx: `frame-ancestors *` вместо `X-Frame-Options`; ADR 42; `buildContext` вынесен в `lib/chatUtils.js` | (этап 28, текущий) |
...
- **Публичные агрегаты по студентам** (`db/04_views.sql`, ADR 41): вью `v_*`
  (численность без PII) видны всем, включая гостя; `FORCE` RLS снят с
  `students` (владелец считает агрегаты), `marks` — с `FORCE`.
...
- Пакет **`app`**: `/ask` — **тул-агент** со стримингом (NDJSON:
  ...
  реальная роль.
- Фронтенд: **встраиваемый виджет** — страница `/widget` (плавающий
  launcher-чат для `<iframe>` на внешних сайтах университета, ADR 42), гость
  + кнопка «Войти» (вход в новой вкладке, сессия синхронизируется через
  `storage`-событие), API-базис из собственного origin, nginx разрешает
  встраивание (`frame-ancestors *` вместо `X-Frame-Options`).
