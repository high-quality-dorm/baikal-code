...
- Подключение гостевого чата к реальному `/ask` во фронтенде и отрисовка
  NDJSON-потока агента (бэкенд уже отдаёт `status`/`query`/`token`/`done`;
  сейчас гость отвечает через mock). Фронтенд, вероятно, перепишется.
