...
| 25 | Приоритет должности в resolve_role | В `resolve_role` известная должность (`teacher/head/dean/admin`) приоритетнее `student`; неизвестная должность не маскирует студента; доступ (RLS) не меняется — только представление роли; ADR 39; тесты (197) | (этап 25, текущий) |
...
- **Устранение замечаний бэкенда** (поэтапный план — [fixes.md](fixes.md)):
  rate limit, PII по RLS-скоупу и приоритет должности в `resolve_role` —
  выполнены (этапы 23–25); осталось — реальная роль в аудите.
