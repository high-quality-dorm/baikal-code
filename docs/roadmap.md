...
| 24 | PII по RLS-скоупу | `AuthContext.can_see_pii = resolve_identity != None` (есть RLS-скоуп, а не роль); проброс через `Agent.stream` в `build_system_prompt(schema_text, role, can_see_pii)`: True — PII разрешён в рамках скоупа, False (гость) — «только агрегаты»; RLS/схема не меняются; ADR 38; тесты (195) | (этап 24, текущий) |
...
- **Устранение замечаний бэкенда** (поэтапный план — [fixes.md](fixes.md)):
  rate limit и PII по RLS-скоупу — выполнены (этапы 23/24); осталось —
  приоритет должности в `resolve_role`, реальная роль в аудите.
...
- Пакет **`app`**: `/ask` — **тул-агент** со стримингом (NDJSON:
  ...
  единый контейнер сервисов и шов DI.
- `make format`/`check`/`test` работают по `packages/db` + `packages/app`
  (`pytest tests/` — 181 тест). Реальные значения LLM заполняются в `.env`
  вручную.