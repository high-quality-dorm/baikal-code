...
- **Устранение замечаний бэкенда** — **все выполнены** (этапы 23–26): rate limit
  (ADR 37), PII по RLS-скоупу (ADR 38), приоритет должности в `resolve_role`
  (ADR 39) и реальная роль в аудите (ADR 16).
...
- Пакет **`app`**: `/ask` — **тул-агент** со стримингом (NDJSON:
  ...
  23–26): `/ask` ограничен rate limiting (ADR 37),
...
Проект покрывает все основные направления конкурсной оценки решения:

- **Техническая архитектура (20):** асинхронная обработка запросов (FastAPI +
  asyncpg), параллелизм (пулы соединений по ролям `app_ro`/`app_service`),
  логирование всех действий пользователя (`query_log` аудит, ADR 16),
  тестирование корректности и стабильности (`pytest tests/` — 199 тестов).
- **Решение бизнес-кейса (30):** полный цикл «вопрос → SQL → выполнение →
  ответ» на естественном языке (тул-агент, ADR 36), корректная генерация SQL
  с JOIN и агрегатами, соблюдение политики персональных данных (RLS, ADR 38,
  ADR 41), встраиваемый чат-виджет для интеграции в цифровые ресурсы
  университета (ADR 42).
- **Производительность (15):** контроль объёма выборки (авто-LIMIT,
  `MAX_ROWS = 200`, флаг `truncated`), скорость ответа в цепочке
  вопрос → LLM → БД → ответ, устойчивость под нагрузкой (rate limiting, ADR 37).
- **Безопасность (20):** только read-only SELECT (валидация sqlglot),
  whitelist таблиц, защита от SQL-инъекций, `statement_timeout`, соблюдение
  политики персональных данных (RLS deny-by-default), публичные агрегатные
  вью `v_*` без PII (ADR 41).
- **Бонус Explainable AI (+10):** структурированный блок «Объяснение запроса»
  в ответе агента (таблицы, JOIN, фильтры, агрегаты, лимит) + раскрытие SQL
  и метаданных в интерфейсе.
- **Бонус Big Data (+10):** интеллектуальное управление выборкой — авто-LIMIT,
  анализ потенциального объёма, предупреждение о широком запросе и предложение
  уточняющих фильтров, устойчивость к нагрузке.